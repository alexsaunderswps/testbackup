...
> **Key API facts (discovered session):**
...
- [x] ~~**`test_api_installations.py`** — GET list: status 200, plain array (note: NOT ResponseDto)~~ ✅ Completed: `test_get_list_returns_200`, `test_get_list_returns_array`
- [x] ~~**GET list items have required fields**~~ ✅ Completed: `test_get_list_items_have_required_fields` (installationId, name, organizationId)
- [x] ~~**GET pagination**~~ ✅ Completed: `test_get_list_page_size_param_limits_results` (pageSize=1 respected); full multi-page math deferred — needs sufficient QA data
...
- [ ] **Schema validation** — GET list and detail schemas (deferred to Section 10)
...
- [x] ~~**Fix `search_catalogues()` race condition in `video_catalogues_page.py`** — `expect_response` exits when the HTTP response arrives, but React processes it asynchronously; `count_table_rows()` read stale DOM (12 rows) before re-render completed, causing `test_video_catalogue_search_with_no_match_shows_empty_table` to fail~~ ✅ Completed 2026-02-24 (Session D): Added `wait_for_load_state("networkidle")` after `expect_response` in `search_catalogues()`; both waits are required — `expect_response` guards against capturing a prior in-flight response, `networkidle` waits for React to finish updating the DOM
